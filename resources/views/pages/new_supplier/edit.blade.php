...
<section class="content">
 ...
 <div class="modal-body">
            ...
            <form method="POST" action="{{route('new_supplier.update',encrypt($supplier->id))}}" enctype="multipart/form-data">
            {{method_field('PATCH ')}}
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control"  aria-describedby="emailHelp" placeholder="Enter name" name="name_ar" value="{{$supplier->name_ar}}"  id="regax_name_ar" onkeyup="check_regax_name_ar();" onkeypress="return CheckArabicCharactersOnly(event);"   required oninvalid="this.setCustomValidity('يجب ان يكون اسم المورد باللغة العربية وايضا لا يكون ارقام فقط')"  oninput="this.setCustomValidity('')">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter name" name="name_en" value="{{$supplier->name_en}}"  required onkeypress="return CheckEnglishCharactersOnly(event);"  oninvalid="this.setCustomValidity('يجب ان يكون اسم المورد باللغة الانجليزية وايضا لا يكون ارقام فقط')"  oninput="this.setCustomValidity('')">
              ...
              <!----------------------------------------------------->
                <div class="form-group">
                    ...
                    @if($supplier->getFirstMediaUrl('supplier','edit'))
                         <center><img id="previewImg"src="{{$supplier->getFirstMediaUrl('supplier','edit')}}" class="uploaded-img"> </center>
                         <input type="hidden" name="media_url" value="{{$supplier->getFirstMediaUrl('supplier')}}">
                     @else
                ...
                <input type="hidden" name="id" value="{{encrypt($supplier->id)}}">
