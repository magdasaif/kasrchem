...
;(function($){ "use strict";
    ...
    Iconpicker.prototype.updateSearch = function () {
        ...
        var search = [
            '<tr>',
            '   <td colspan="' + op.cols + '">',
            '       <input type="text" name="icon_from_js" class="form-control search-control" style="width: ' + op.cols * (($.fn.bsVersion() === '3.x') ? 39 : 41) + 'px;" placeholder="' + op.searchText + '">',
            '   </td>',
            '</tr>'
        ];
        search = $(search.join(''));
    ...
    Iconpicker.prototype.setSearchText = function (value) {
        ...
        this.options.table.find('.search-control').attr('name', 'nnnnn');
